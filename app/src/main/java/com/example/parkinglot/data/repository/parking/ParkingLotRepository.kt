...
import com.example.parkinglot.domain.model.kakao.Document
...
import com.example.parkinglot.data.network.kakao.KakaoLocalRetrofitClient
import com.example.parkinglot.data.network.parking.ParkingLotRetrofitClient
        ...
        details.forEach { lot ->
            ...
            if (best != null) {
                ...
                if (lat != 0.0 && lon != 0.0) {
                    ...
                    successCount++
                    Log.v("ParkingRepo", "좌표 성공: ${lot.id} -> ($lat, $lon)")
                } else {
                    failCount++
                    Log.w("ParkingRepo", "좌표 파싱 실패: ${lot.id} -> lat=$lat, lon=$lon")
                }
            } else {
                failCount++
                Log.w("ParkingRepo", "주소 검색 실패: ${lot.id}")
            }
        }

        Log.d("ParkingRepo", "좌표 매핑 완료 - 성공: $successCount, 실패: $failCount")
        Log.d("ParkingRepo", "=== 구별 주차장 조회 완료 ===")
